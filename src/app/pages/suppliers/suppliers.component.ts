    ...
    <section class="pt-36 pb-12 px-6 lg:px-16 xl:px-20 bg-[#0A0A0A]">
      <div class="max-w-7xl mx-auto">
        ...
        <h1 class="font-display font-black text-[#F5F0EB] uppercase leading-[0.95] tracking-[-0.02em]"
          ...
          Nuestros<br><span class="text-[#C9A84C]">proveedores.</span>
        </h1>
    ...
    <section class="bg-[#0A0A0A] px-6 lg:px-16 xl:px-20 sticky top-[68px] z-[var(--z-sticky)] border-b border-[#F5F0EB]/8">
      <div class="max-w-7xl mx-auto flex gap-6 lg:gap-8 overflow-x-auto" role="tablist">
        <button (click)="setCategory(null)"
                ...
                class="gallery-tab text-[11px] tracking-[0.22em] uppercase py-4 whitespace-nowrap"
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div *ngFor="let sup of filteredSuppliers()"
               class="reveal group relative overflow-hidden bg-[#111111] border border-[#F5F0EB]/8 flex flex-col">
            <div class="relative overflow-hidden h-[200px] img-zoom">
              <img [src]="sup.imageUrl" [alt]="sup.name" class="w-full h-full object-cover">
              <span class="absolute top-3 left-3 bg-[#0A0A0A]/80 text-[#C9A84C] text-[9px] font-bold tracking-[0.2em] uppercase px-2.5 py-1">
                {{ categoryLabel(sup.category) }}
              </span>
            </div>
            <div class="p-6 flex flex-col flex-1">
              <h3 class="font-display font-black text-[#F5F0EB] uppercase text-lg tracking-tight mb-2">
                {{ sup.name }}
              </h3>
              <p class="text-[#F5F0EB]/50 text-[13px] font-light leading-relaxed mb-4 flex-1">
                {{ sup.description }}
              </p>
              <div class="flex items-center justify-between border-t border-[#F5F0EB]/8 pt-4">
                <p class="text-[#C9A84C] text-sm font-bold tabular-nums">
                ...
                <a [href]="'https://wa.me/52' + sup.phone" target="_blank" rel="noopener noreferrer"
                   class="btn-grand w-9 h-9 flex items-center justify-center border border-[#C9A84C]/30 text-[#C9A84C] hover:bg-[#C9A84C] hover:text-[#0A0A0A]"
